...
struct viaje {
    ...
    int origen_dest_next; // número de lineas abajo hasta encontrar mismo origen y mismo destino
...
int main () {
    ...
    int origen, destino, origen_dest_next, count = 0;
    fpos_t pos;
    char c = 'a';
	...
	struct viaje nextreg;
	    ...
	    origen_dest_next++;
            ...
            if (nextreg.origen == origen && nextreg.destino == destino)
		break;
			
        }
		
	reg.origen_dest_next = origen_dest_next;
